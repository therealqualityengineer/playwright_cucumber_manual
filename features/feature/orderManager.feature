Feature: Order Manager Functionality

  Background:
    Given the user login to the application 'Env_QA' with 'testuser_03' credentials

  @regression @43455
  Scenario: Create a new order
    And the user create a new temp with the following details
      | Field                     | Value             |
      | First Name                | <RandomAlphabets> |
      | Last Name                 | <RandomAlphabets> |
      | Primary Email             | <RandomEmail>     |
      | Address                   |   345 Park Avenue |
      | City                      | New York          |
      | State                     | NY                |
      | ZipCode                   |             10154 |
      | Status                    | Active            |
      | Region                    | JasonTest         |
      | Contract (1099) / EE (W2) | EE                |
      | Certification             | RN                |
      | Specialty                 | ER                |
    Then the temp id should be generated successfully in the url
    Given the user create a new client with the following details
      | Field        | Value              |
      | ClientName   | <RandomAlphabets>  |
      | Address      | 16801 Addison Road |
      | City         | Addison            |
      | State        | TX                 |
      | ZipCode      |              75001 |
      | Status       | Active             |
      | Region       | JasonTest          |
      | QuickBooksID | <RandomNumbers>    |
    Then the client id should be generated successfully in the url
    Given the user create a new order with the following details
      | Field      | Value                |
      | Client     | <this.clientName>    |
      | Temp       | <this.tempFirstName> |
      | Job Date   | <Today+2>            |
      | shiftnum   |                  108 |
      | Order Cert | RN                   |
      | Specialty  | ER                   |
    Then the order id should be generated successfully
