...
Given(
  "the user create a new temp with the following details",
...
Then(
  "the temp id should be generated successfully in the url",
...
Given(
  "the user added the Flat pay of {string} and {string} to Pay and Bill amounts",
  async function (this: CustomWorld, pay: string, bill: string) {
    await this.tempManagerPage.navigateTo(
      `/wfportal/temppay.cfm?tempid=${this.tempId}`,
    );
...
When(
  "the user opens the {string} profile page",
  async function (this: CustomWorld, profileType: string) {
    if (profileType === "temp") {
      await this.tempManagerPage.navigateToProfile(this.tempId ?? "");
    } else if (profileType === "client") {
      await this.clientManagerPage.navigateToProfile(this.clientId ?? "");
    }
  },
);

Given(
  "the user opens the {string} tab and applies the following filters",
  async function (this: CustomWorld, tab: string, dataTable: DataTable) {
    if (tab === "Facilities") {
...
Given(
  "the user sets the following status on the Facilities page",
  async function (this: CustomWorld, dataTable: DataTable) {
    for (const row of dataTable.raw().slice(1)) {
      const field = row[0] ?? "";
      const value = row[1] ?? "";
...
Then(
  "the user verifies the {string} message",
...
Then(
  "the user verifies that the following status is set on the {string} page",
  async function (this: CustomWorld, _page: string, dataTable: DataTable) {
    for (const row of dataTable.raw().slice(1)) {
      const field = row[0] ?? "";
      const status = row[1] ?? "";
...
Then(
  "the user verifies the {string} Driving Distance should be between {string} to {string}",
  async function (
    this: CustomWorld,
    type: string,
    minMiles: string,
    ...
    await this.tempManagerPage.verifyDrivingDistanceBetween(
      type,
      parseInt(minMiles, 10),
      parseInt(maxMiles, 10),
    );
  },
);
